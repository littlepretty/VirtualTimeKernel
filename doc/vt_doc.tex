...
\begin{itemize}
	...
	\item \texttt{freeze\_start\_ns} represents the starting moment that a process or process group is frozen. It is always zero when the process is not frozen. The process's group leader is responsible for maintaining this variable for its members.
	\item \texttt{freeze\_past\_ns} represents the accumulative time, in nanoseconds, that a running process or process group was put into freezing state. The process's group leader is responsible for maintaining this variable for its members.
\end{itemize}
...
As we will see in subsection \ref{Alg-Freeze}, how long a process is frozen, the time it should not perceived, is stored at its group leader's $freeze\_past\_ns$.
\end{itemize}
...
\ElsIf{$new\_tdf > 0$}\Comment{real works here}
	...
	\State $delta\_ppn \gets now \; - \; tsk.physical\_past\_ns \; - \; tsk. physical\_start\_ns \; - \; tsk. group\_leader.freeze\_past\_ns$
	\State $delta\_vpn \gets delta\_ppn \; / \; old\_tdf$
	\State $tsk.virtual\_past\_ns \; += \; delta\_vpn$
	\State $tsk.physical\_start\_ns \gets (now \; - \; tsk.group\_leader.freeze\_past\_ns)$
...
The \textit{threadgroup leader} deals with the freeze part of time keeping for all its members. 
Besides, freeze and unfreeze should be done with respect to a entire process group. This design is based on two assumptions
\begin{itemize}
\item Freeze usually involves the entire process group. As an illustration, consider the case that we wants to freeze a container in the network emulation. Recall that container here usually plays the role of a lightweight virtual machine. Therefore when we want to freeze this container, we actually wants to freeze all the processes(usually emulates applications running inside a VM) inside this container. For example, when we freeze host $h_1$(container), we should also stop the clock of \texttt{ping}, \texttt{iperf} or other applications running inside $h1$. These applications, it turns out, have the same leader $h1$.
\item Another more general assumption is that when a process is frozen, it is unable to query time or do anything else because it is put into $STOP$ status. 
Under this assumption, we can ensure that every member process will access the same frozen duration.
\end{itemize}
So instead of let every process maintain the frozen time and, maybe, communicate with other members in the same group for consistency, we make the leader a central controller of the freeze time keeping. 
...
The drawback is that we waste 16 bytes for every process other than the leader, who satisfying $tsk.group\_leader==tsk$.

We must admit that the above assumptions, and thus our virtual time's freeze implementation, is highly tailored for the purpose of container based emulation. After all this is our motivation of implementing freeze/unfreeze inside virtual time. 
...
The implementations for both granularity, however, appears to be somewhat similar. 
First, after $kill$ a group of processes, we make a record of current moment; this record is used to calculate how long this group is frozen when we unfreeze it. It is a little bit subtle that when unfreeze, $kill$ operation is behind time keeping. 
The reason is that if we resume the process group first, by any chance, an unfreeze process may be schedule to run and query time before we write the $freeze\_past\_ns$ to group leader.
...
\State $now \gets$ \texttt{timespec\_to\_ns}($ts$)
\State $tsk.group\_leader.freeze\_start\_ns \gets now$
\EndFunction
\\
\Function{Unfreeze}{tsk}
\State \texttt{\_\_getnstimeofday}(\&$ts$)\Comment{\texttt{timespec} $ts$}
\State $now \gets$ \texttt{timespec\_to\_ns}($ts$)
\State $tsk.group\_leader.freeze\_past\_ns \; += \; now \; - \; tsk.group\_leader.freeze\_start\_ns$
\State $tsk.group\_leader.freeze\_start\_ns \gets 0$
...
\begin{itemize}
\item \texttt{clean\_up\_virtual\_time} reset all relevant variables to zero, which is also the default value for these variables when create a \texttt{task\_struct}
...
\State $leader \gets tsk.group\_leader$
\State $now \gets \texttt{timespec\_to\_ns($ts$)}$
